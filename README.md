...
```
   ...
   LLM_MODEL=
```
5. Download the eBook and save it as `data/Ebook-Agentic-AI.pdf`.
6. Run ingestion **once**. This chunks the PDF, creates the embeddings and stores them in Pinecone:
```bash
   python -m app.ingest
```
   Ingestion takes a couple of minutes because it is rate-limited (see Limitations). Running it again is safe, chunks have fixed IDs and are overwritten, not duplicated.
7. Start the server:
```bash
...
- **Two-layer grounding:**
  1. If the top similarity score is below the threshold, the LLM is not called at all and a fixed fallback message is returned.
